<x-app-layout>
    ...
    @endif
    <form action="{{route('evaluasis.index')}}" method="get" enctype="multipart/form-data">
    @csrf
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @foreach ($siswas as $siswa)
                @php
                    ...
                    if (count($siswa->evaluasis) > 0 ) {
                        $data[$count][5] = $siswa->evaluasis->last()->evaluasi;
                        $data[$count][6] = $siswa->evaluasis->last()->keterangan;
                        $data[$count][7] = \Carbon\Carbon::parse($siswa->evaluasis->last()->tanggal)->translatedformat('d F Y');
                    } else {
                        $data[$count][5] = '-';
                        $data[$count][6] = '-';
                        $data[$count][7] = '-';
                    }
                    $data[$count][8] = $siswa->id;
                    $count++;
                    @endphp
            @endforeach
                        ...
                        <tbody class="flex-1 sm:flex-none">
                            @for ($i = 0; $i < $count; $i++)
                                <tr
                                    ...
                                    <td
                                        class="truncate w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
                                        <span
